      ...
      <header className="px-8 pt-8 pb-4 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 flex items-center gap-2">
            <Share2 size={20} className="text-brand-500" /> {t('shared.title')}
          </h1>
          <p className="text-sm text-slate-500 mt-1">{t('shared.subtitle')}</p>
        </div>
        <a className="btn-outline" href="/api/export/shared-boms.csv" download title={t('btn.export')}>
      ...
      <div className="px-8 flex flex-wrap gap-2 mb-3">
        {groups.map((g) => (
          ...
          >
            ...
            {g.channel === 'takeout' ? t('chan.takeout') : g.channel === 'dinein' ? t('chan.dinein') : t('chan.generic')}
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h3 className="font-semibold text-slate-900 flex items-center gap-1.5">
            ...
            {group.channel === 'takeout' ? t('chan.takeout') : group.channel === 'dinein' ? t('chan.dinein') : t('chan.generic')}
          </h3>
          <button
          ...
          >
            <Power size={11} /> {enabled ? t('shared.enabled') : t('shared.disabled')}
          </button>
          <span className="text-xs text-slate-400">
            {group.channel === 'takeout' && t('shared.trigger_takeout')}
            {group.channel === 'dinein'  && t('shared.trigger_dinein')}
            {!group.channel && t('shared.trigger_all')}
          </span>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100">
        ...
        >
          <option value="">{t('shared.add_line')}</option>
          {materials
