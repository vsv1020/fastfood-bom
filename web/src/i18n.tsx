...
const D: Dict = {
  ...
  'nav.margin':     { zh: '毛利',       en: 'Margin',      th: 'กำไร' },
  'nav.settings':   { zh: '设置',       en: 'Settings',    th: 'ตั้งค่า' },
  // margin page
  'margin.title':       { zh: '毛利率',     en: 'Gross Margin', th: 'อัตรากำไรขั้นต้น' },
  'margin.subtitle':    { zh: '每个套餐堂食 / 外卖的成本与毛利率', en: 'Per-set dine-in / takeout cost and margin', th: 'ต้นทุนและกำไรของแต่ละชุด (ทานในร้าน / กลับบ้าน)' },
  'margin.col.code':    { zh: '编码',       en: 'Code',         th: 'รหัส' },
  'margin.col.name':    { zh: '名称',       en: 'Name',         th: 'ชื่อ' },
  'margin.col.folder':  { zh: '文件夹',     en: 'Folder',       th: 'โฟลเดอร์' },
  'margin.col.to_price':{ zh: '外卖售价',   en: 'Takeout price', th: 'ราคากลับบ้าน' },
  'margin.col.to_cost': { zh: '外卖成本',   en: 'Takeout cost',  th: 'ต้นทุนกลับบ้าน' },
  'margin.col.to_margin':{ zh: '外卖毛利率', en: 'Takeout margin', th: 'กำไรกลับบ้าน' },
  'margin.col.di_price':{ zh: '堂食售价',   en: 'Dine-in price', th: 'ราคาทานในร้าน' },
  'margin.col.di_cost': { zh: '堂食成本',   en: 'Dine-in cost',  th: 'ต้นทุนทานในร้าน' },
  'margin.col.di_margin':{ zh: '堂食毛利率', en: 'Dine-in margin', th: 'กำไรทานในร้าน' },
  'margin.no_price':    { zh: '未设售价',   en: 'No price',      th: 'ยังไม่ตั้งราคา' },
  'margin.missing_prefix': { zh: '缺价物料: ', en: 'Missing price: ', th: 'ขาดราคา: ' },
  'margin.formula':     {
    zh: '成本 = 内部价 ×(1 + 加成 {markup}%)×(1 + 物料 ERP 税率);价已含税(VAT Included)则不乘税。税率按各物料的 ERP Item Tax Template 自动取。',
    en: 'Cost = internal price ×(1 + markup {markup}%)×(1 + item ERP tax rate); if tax-inclusive (VAT Included), tax is not applied. Tax rate is taken automatically from each item\'s ERP Item Tax Template.',
    th: 'ต้นทุน = ราคาภายใน ×(1 + มาร์กอัป {markup}%)×(1 + อัตราภาษี ERP ของวัตถุดิบ); หากรวมภาษีแล้ว (VAT Included) จะไม่คูณภาษี อัตราภาษีดึงจาก ERP Item Tax Template ของแต่ละวัตถุดิบโดยอัตโนมัติ',
  },
  'margin.tax_rate':    { zh: '税率',       en: 'Tax rate',     th: 'อัตราภาษี' },
  'margin.markup':      { zh: '加成',       en: 'Markup',       th: 'มาร์กอัป' },
  'margin.sync_prices': { zh: '同步 Selling-Internal 价格', en: 'Sync Selling-Internal prices', th: 'ซิงค์ราคา Selling-Internal' },
  'margin.sync_done':   { zh: '匹配 {matched} 条,缺价 {missing} 条', en: 'Matched {matched}, missing {missing}', th: 'จับคู่ {matched} ขาด {missing}' },
  'margin.empty':       { zh: '暂无套餐', en: 'No sets',        th: 'ไม่มีชุด' },
  // common buttons
  ...
  'export.margin_xlsx': { zh: '毛利明细 (Excel)', en: 'Margin detail (Excel)', th: 'รายละเอียดกำไร (Excel)' },
  ...
  'editor.price_dinein':  { zh: '堂食售价', en: 'Dine-in price', th: 'ราคาทานในร้าน' },
  'editor.price_takeout': { zh: '外卖售价', en: 'Takeout price', th: 'ราคากลับบ้าน' },
  'editor.sale_price': { zh: '售价', en: 'Price', th: 'ราคา' },
  'mat.price_includes_tax': { zh: '含税', en: 'Tax-inclusive', th: 'รวมภาษี' },
  'mat.internal_price':     { zh: '内部价', en: 'Internal price', th: 'ราคาภายใน' },
  'mat.tax_rate':           { zh: '税率', en: 'Tax rate', th: 'อัตราภาษี' },
  'mat.tax_included':       { zh: '价含税(VAT Included)', en: 'Tax-inclusive (VAT Included)', th: 'รวมภาษี (VAT Included)' },
  'mat.tax_excluded':       { zh: '价不含税', en: 'Tax-exclusive', th: 'ไม่รวมภาษี' },
  'mat.tax_from_erp':       { zh: '税务信息从 ERP 自动同步,不可手动修改', en: 'Tax info is auto-synced from ERP and not editable', th: 'ข้อมูลภาษีซิงค์จาก ERP อัตโนมัติ แก้ไขไม่ได้' },
  'mat.synced_at':          { zh: '同步时间', en: 'Synced at', th: 'ซิงค์เมื่อ' },
  ...
  // combo margin card
  'combo.pricing_title':    { zh: '定价与毛利率(成本基于已保存配置)', en: 'Pricing & margin (cost from saved config)', th: 'ราคาและกำไร (ต้นทุนจากค่าที่บันทึก)' },
  'combo.margin_title':     { zh: '毛利率',    en: 'Margin',    th: 'กำไรขั้นต้น' },
  'combo.ch_takeout':       { zh: '外卖',      en: 'Takeout',   th: 'เดลิเวอรี' },
  'combo.ch_dinein':        { zh: '堂食',      en: 'Dine-in',   th: 'ทานในร้าน' },
  'combo.takeout_margin':   { zh: '外卖毛利',  en: 'Takeout',   th: 'เดลิเวอรี' },
  'combo.dinein_margin':    { zh: '堂食毛利',  en: 'Dine-in',   th: 'ทานในร้าน' },
  'combo.fill_price':       { zh: '请填写售价', en: 'Enter price', th: 'กรอกราคาขาย' },
  'combo.margin_after_save':{ zh: '保存后显示毛利率', en: 'Save to see margin', th: 'บันทึกเพื่อดูกำไร' },
  'combo.cost_label':       { zh: '成本',      en: 'Cost',      th: 'ต้นทุน' },
  'combo.margin_hint':      { zh: '成本基于已保存配置', en: 'Cost from saved config', th: 'ต้นทุนจากการตั้งค่าที่บันทึกไว้' },
